...
export default function User() {
  ...
  const getAllModels = async () => {
    ...
    response.json().then((res) => setMylist(res));
  };
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - mylist.length) : 0;

  const filteredUsers = applySortFilter(mylist, getComparator(order, orderBy), filterName);
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            İstifadəçilər haqqında məlumatlar
          </Typography>
